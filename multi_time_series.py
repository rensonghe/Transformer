...
class TransAm(nn.Module):
    def __init__(self, feature_size=10, num_layers=3, dropout=0.1):  #feature_size为多少列时间序列
...
data_ = pd.read_csv('data_2min_bar_2020_2021_new.csv')
data_ = data_.drop(['buyratio','sellratio','buyprice','sellprice',
                    'tradingprice','tradingvolume','time'],axis=1)
data_ = data_[~data_['close'].isin([0])]
...
def get_data():
    ...
    # sampels = 2800
    train_data = amplitude[:10000]
    test_data = amplitude[-2000:]
    ...
    for batch, i in enumerate(range(0, len(train_data) - 1, batch_size)):
        ...
        if batch % log_interval == 0 and batch > 0:
            ...
            print('| epoch {:3d} | {:5d}/{:5d} batches | '
                  'lr {:02.6f} | {:5.2f} ms | '
                  'loss {:5.5f} | ppl {:8.2f}'
                .format(epoch, batch, len(train_data) // batch_size, scheduler.get_lr()[0],
...
def plot(eval_model, data_source, epoch, scaler):
    ...
    # len(test_result)
    # predict_close = test_result[:,-1]
    # truth_close = truth[:,-1]
    # predict_col_close = inverse_transform_col(scaler, predict_close, n_col=-1)
    # truth_col_close = inverse_transform_col(scaler, truth_close, n_col=-1)
    #
    # pyplot.plot(truth_col_close[-100:], color='ground truth')
    # pyplot.plot(predict_col_close[-100:], color='predict')
    # pyplot.title('Close Prediction')
    # pyplot.xlabel('Time')
    # pyplot.ylabel('Price')
    # pyplot.legend()
    # pyplot.savefig('predict_2021.png')
    # pyplot.show()

    # print(test_result)
    test_result_ = scaler.inverse_transform(test_result[:700])
    truth_ = scaler.inverse_transform(truth)
    # print(test_result_.shape)
    # print(truth_)
    for m in range(9):
